...
## 프로젝트 소개
Flutter와 Firebase를 활용하여 개발한 크로스플랫폼 실시간 채팅 앱입니다.  
이 앱은 AOS(Android)와 iOS에서 모두 동작하며, 다음과 같은 특징을 갖고 있습니다:

- 1:1 채팅 및 그룹 채팅 기능 지원
- 사용자 맞춤 채팅방 커스텀 기능 (배경, 말풍선, 글씨 크기 조절)
- Firebase 인증을 통한 보안 강화된 사용자 로그인 및 이메일 인증
- 멀티미디어 파일 전송 기능 (이미지, 동영상)
- 사용자 검색 및 친구 추가 기능

이 앱은 Firebase Firestore와 Firebase Storage를 활용하여 실시간 데이터 동기화와 파일 저장을 지원하며,  
최적화된 UI/UX로 원활한 사용자 경험을 제공합니다.
...
- 이메일 인증 기능
- 계정 찾기 및 비밀번호 재설정 기능
- Firebase Remote Config 활용하여 앱 설정 업데이트 가능
